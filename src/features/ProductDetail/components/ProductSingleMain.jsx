...
const Variant = styled.button`
  ...
  border: ${(props) =>
    props.$active ? "2px solid #0a68ff" : "2px solid rgba(0,0,0,0)"};
...
export default function ProductSingleMain({ data, variant, request, star }) {
  ...
  const [variantAttributes, setVariantAttributes] = useState(
    new Array(variant.length).fill(null)
  );
  ...
  const onGetVariant = () => {
    return data.variants.find((item) => {
      ...
      for (i; i < item.variantAttributes.length; i++) {
        if (
          item.variantAttributes[i].attributeValue !=
          variant[i].values[variantAttributes[i]]
        ) {
          break;
        }
      }
      ...
      createCartItemRequest.mutate(formData, {
        onSuccess: (response) => {
          ...
          if (response.status == 405) {
            setErrorMessage(
              `The remaining quantity of this product is ${response.data.quanity}.`
            );
            setCartError(true);
            request.refetch();
          }
  ...
  useEffect(() => {
    if (
      ...
      onGetVariant().availableQuanity == 0
    ) {
      setQuantity(0);
    } else {
        ...
        <ShipmentDetail>
          <StyledShippingBoxWrap>
            <div>
              <h4>Shipping Fee</h4>
              <ul>
                <li>Ha Noi city - 3.00 USD</li>
                <li>The remaining provinces - 5.00 USD</li>
              </ul>
            </div>
            <div>
